...
void tlCallback(byte slotIdx, byte progress); //Prototype, fully declared later

TPin led;
TButton button;
TTimelineT<byte> tline(tlCallback, 2);
byte currentSlot = 0;

void tlCallback(byte slotIdx, byte value)
{
  //Set the PWM value to the LED
  if (slotIdx == 0) led.pwm(value); //Fade in
  else led.pwm(255 - value); //Fade out, invert progress
}
